...
class blood:
    def __init__(self,x,y,offset,dist):
        ...
        self.dist = dist
        self.offset = offset
    def existence(self, bloodsprites,i,frametime):
        self.bloodtime += 4*frametime
...
class Monster:
    ...
    def move(self,playerx,playery,playerrad,sprites,znum,res,frameTime):
            ...
            if fullhyp >= playerrad and self.findingpath:
                newx = (self.x+newo*frameTime)
                newy = (self.y+newa*frameTime)
                move=True
                ...
                if self.x > destx -0.2 and self.x < destx +0.2:
                    if self.y > desty -0.2 and self.y < desty +0.2:
                        if len(self.path) > 1:
                            del self.path[0]
            self.pathfindcooldown -= frameTime*30
...
def mapeditor(mapwidth):
 ...
 while True:   
    ...
    textsurface = myfont.render(('SPC'), False, (255,255,255))
    screen.blit(textsurface,(int(7*(width/8)),int(0.1*0.1*height)))
    for event in pygame.event.get():
...
def main(posX,posY,worldMap,resolution):
    ...
    for x in range(len(worldMap)):
        for y in range(len(worldMap[x])):
            ...
            if worldMap[x][y] == 7:
                ...
                monsty = Monster(x+0.5,y+0.5,1,random.randint(4,6),random.randint(50,100)/100,zombieimgs,bloods,0)
    ...
    sky = pygame.image.load('night.jpg').convert()
    sky = pygame.transform.scale(sky,(700,360))
    sky = pygame.surfarray.array3d(pygame.transform.scale(sky,(360,halfvres*2)))
    floor = pygame.surfarray.array3d(pygame.transform.scale(pygame.image.load('floor.jpg').convert(),(100,100)))
    #a lot of variables
    health = 5  
    gameover = False
    ...
    pistolshootsound = pygame.mixer.Sound("pistolshoot.wav")
    pistolshootsound.set_volume(0.25)
    pistolemptysound = pygame.mixer.Sound("empty.mp3")
    pistolemptysound.set_volume(1.0)
    pistolreloadsound = pygame.mixer.Sound('pistolreload.wav')
    pistolreloadsound.set_volume(1.0)
    floorvisible = True
    walltexturevisible = True
    wait = 0
   
    while True:
        if wait < 3*60*frameTime:
            wait+=1
        num_of_zombies = 0
        ...
        if floorvisible:
            ...
            ceiling = pygame.transform.flip(surf,False,True)
            screen.blit(surf,(0,height/2),(0,height/2,width,height/2))
            screen.blit(ceiling,(0,0),(0,0,width,height/2))

        startx = 0
        amount = 0
        if walltexturevisible:
            for x in range(0,(width+resolution),resolution):
                #got the maths from lodedevs tutorial
            ...
            for x in range(0,(width),resolution):
                ...
                yellow = ((255,255,0))
                color = white
                if worldMap[int(mapX)][int(mapY)] == 1:
                    color = red
                elif worldMap[int(mapX)][int(mapY)] == 2:
                    color = green
                elif worldMap[int(mapX)][int(mapY)] == 3:
                    color = blue
                elif worldMap[int(mapX)][int(mapY)] == 4:
                    color = white
                elif worldMap[int(mapX)][int(mapY)] == 5:
                    color = yellow
                    # if tx > posX -5 and tx < posX + 5 and ty > posY - 5 and ty < posY +5:
                shade = int(perpWallDist/3)
                if shade<=0:
                    shade = 1
                color = (int(color[0]/shade)>>1,int(color[1]/shade)>>1,int(color[2]/shade)>>1)
                if side == 1:
                    color = (color[0]>>1,color[1]>>1,color[2]>>1)
                pygame.draw.line(screen,color,(x,drawStart),(x,drawEnd),resolution)
                for i in range(resolution):
                    walldists.append(perpWallDist)
                    wallcols.append(color)
        checks = 1
        ...
        if len(bloodsprites) > 0:
            for i in range(len(bloodsprites)-1,-1,-1):
                bloodsprites[i].existence(bloodsprites,i,frameTime)
                if len(bloodsprites) <= 0:
                    break
        ...
        if not gameover:
            ...
            pygame.draw.rect(screen,(255,0,0),(width/2,2,width/10*health,height/20))            
        ...
        for event in pygame.event.get():
            ...
            #handle key presses
                ...
                if gameover:
                    if event.key == K_m:
                        file = open('maze1.txt','r')
                        oldworldMap = file.readlines()
                        #function for splitting words

                        #using function to create a 2d list for the world map
                        for i in range(len(oldworldMap)):
                            charlist = split(oldworldMap[i])
                            oldworldMap[i] = charlist
                        title(oldposx,oldposy,oldworldMap)
                    elif event.key == K_SPACE:
                        main()
                if event.key == K_UP:
                    resolution+=1
                ...
                if event.key == K_f:
                    if floorvisible == True:
                        ...
                        floorvisible = True
                if event.key == K_t:
                    if walltexturevisible == True:
                        walltexturevisible = False
                    else:
                        walltexturevisible = True
                if event.key == K_ESCAPE:
                    pygame.quit()
        ...
        if reload and not gameover and unloaded_ammo>0 and ammocount < maxammo and reloadtime < pygame.time.get_ticks() - reloadspeed:
            pygame.mixer.Sound.play(pistolreloadsound)
            ...
            reload = False
            reloadtime = pygame.time.get_ticks()
        if shoot and not gameover and shoottime < pygame.time.get_ticks() - gunspeed and reloadtime < pygame.time.get_ticks() - reloadspeed:
            if ammocount > 0:
                pygame.mixer.Sound.play(pistolshootsound)
                pygame.mixer.music.stop()
                ...
                pygame.mixer.Sound.play(pistolemptysound)
                pygame.mixer.music.stop()
                shoottime = pygame.time.get_ticks()
        if not gameover and wait >= 3*60*frameTime:
        ...
        if gameover:
            ...
            othertext = myfont.render('SPACE TO RESTART, M FOR MENU, ESCAPE TO QUIT', False, (0,0,0))
            ...
            screen.blit(othertext,(width/5,height*5/6))
        pygame.display.update()
